...
MAX_DB_SIZE = 900 * 1024 * 1024  # 900 ميجابايت (لترك مسافة أمان)
...
# ============================================
# 🗄️ User Management with SQLite + Auto Cleanup
# ============================================
...
class UserManager:
    ...
    def _auto_cleanup(self):
        ...
        try:
            ...
            # 1. حذف المستخدمين غير النشطين (أكثر من 30 يوم)
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            ...
            # 2. حذف المستخدمين منتهي الاشتراك (أكثر من 60 يوم)
            ...
            # 3. إذا كان حجم الملف كبيراً، قم بتنظيفه
            if db_size > 5 * 1024 * 1024:  # 5 ميجابايت
                self._vacuum_db()
                self._backup_db()
    ...
    def _vacuum_db(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            print(f"✅ Database vacuumed: {self._get_db_size() / 1024:.1f} KB")
        except Exception as e:
...
def main():
    ...
    db_size = user_manager.get_db_size()
    if db_size > 0:
        st.sidebar.caption(f"📊 DB Size: {db_size / 1024:.1f} KB")
